...
这个版本的主要目标不是继续沿用 npm 上已经发布过、但 README 已与仓库现状漂移的 `v0.2.2`，而是把 fork 的 npm 包重新对齐到当前仓库快照，并为后续版本继续恢复“一版一内容”的发布约束。

### Packaging

- `codex-cli/package.json` 版本提升到 `0.2.3`
- 根 `README.md` 的 source version marker 同步到 `0.2.3`
- npm 重新发布使用 `0.2.3`，避免继续复用内容已不完全一致的 `0.2.2`
