...
- Unless you are otherwise instructed, prefer the `grep_files` tool for broad repo searches and `read_file` to inspect the matching files.
- When a shell search is more appropriate, prefer `rg` or `rg --files` because `rg` is much faster than alternatives like `grep`. If `rg` is not found, then use alternatives.
