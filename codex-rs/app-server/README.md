...
- `collaborationMode/list` — list available collaboration mode presets (experimental, no pagination). This response still omits built-in developer instructions, but now includes short `description`, `tuiVisible`, `allowsRepoMutation`, `updatePlanAvailable`, `requiresProposedPlanBlock`, `requestUserInputAvailable`, and `streamsProposedPlan` metadata so clients can explain preset behavior without parsing prompts. Clients should either pass `settings.developer_instructions: null` when setting a mode to use Codex's built-in instructions, or provide their own instructions explicitly.
